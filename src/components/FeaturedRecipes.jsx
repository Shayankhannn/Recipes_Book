...
const FeaturedRecipes = () => {
  ...
  const handleSave = (formData) => {
    ...
    ConfirmationDialog({
      ...
      ToastsuccessText: `${
        modalMode === "add" ? "Recipe Added!" : "Recipe Updated!"
      }`,
      ToastcancelText: `${
        modalMode === "add" ? "Addition cancelled!" : "Update cancelled!"
      }`,
      confirmButtonText: `${
        modalMode === "add" ? "Yes, Add it!" : "Yes, Update it!"
      }`,
  ...
  return (
    <section className="mt-10">
      <div className="flex-hero !justify-between flex-col md:flex-row lg:flex-row gap-3 p-4 mb-5">
        <h1 className="lg:text-[40px] md:text-xl text-xl text-primary font-extrabold ">
          Featured Recipes
        </h1>
          ...
          filteredRecipes
            ...
            .map((recipe) => (
              <RecipeCard
                ...
                recipe={recipe}
                key={recipe.id}
                handleDetailModal={() => handleDetailModal(recipe)}
                defaultRecipeImage={defaultRecipeImage}
                defaultAuthorImage={defaultAuthorImage}
              />
            ))
        ) : (
          <p className="no-results font-semibold text-2xl text-red-600">
